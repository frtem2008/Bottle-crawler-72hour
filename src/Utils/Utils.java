...
public class Utils {
    ...
    public static Image getImage(Class cl, String name) {
        try {
            return ImageIO.read(getFileUrl(cl, "Images/" + name));
        } catch (IOException e) {
            e.printStackTrace();
        }
        return null;
    ...
    public static int toScreenX(double x, int playerX) {
        //меняйте, чтобы приблизить или отдалить камеру от левого края экрана
        double xLayout = Main.frameSize.getWidth() / 3;
        return (int) ((xLayout) + (x - playerX));
    }
